              ...
              uint32_t level)
{
  ...
  /*#if 1
  ROS_INFO("Dynamic reconfiguration of 'RPM' -- old value: %.3f",config_.rpm);
  #endif
  config_.rpm = g_rpm; // should be set before callback execution.
#if 1
  ROS_INFO("Dynamic reconfiguration of 'RPM' -- new value: %.3f",config_.rpm);
  #endif*/
}

void VelodyneDriver::set_rpm(const double value) {
  if (value < 100.0 || value > 1200.0)
       this->rpm_ = 600.0;
  else
       this->rpm_ = value;
}

void VelodyneDriver::callback_update_rpm(velodyne_driver::VelodyneNodeConfig &config,
                                         uint32_t level ) {
 #if 1
  ROS_INFO("Dynamic reconfiguration of 'RPM' -- old value: %.3f",config_.rpm);
  #endif
  config_.rpm = this->rpm_; // should be set before callback execution.
#if 1
  ROS_INFO("Dynamic reconfiguration of 'RPM' -- new value: %.3f",config_.rpm);
  #endif                                     
}

void VelodyneDriver::do_dynamic_rpm_reconfig() {
   dynamic_reconfigure::Server<velodyne_driver::VelodyneNodeConfig>::
   CallbackType f;
   f = boost::bind (&VelodyneDriver::callback_update_rpm, this, _1, _2);
   srv2_->setCallback (f); 
}
